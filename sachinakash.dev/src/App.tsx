...
function App() {
  ...
  useEffect(() => {
    const update = () => setProgress(Math.min(1, window.scrollY / Math.max(1, document.documentElement.scrollHeight - window.innerHeight)))
  ...
  return <>
    ...
    <Routes location={location}>
      ...
      <Route path="/404" element={<NotFoundPage onOpenTerminal={openTerminal} />} />
      <Route path="*" element={<NotFoundPage onOpenTerminal={openTerminal} />} />
    </Routes>
    <Footer onOpenTerminal={openTerminal} />
    <Suspense fallback={null}><PortfolioTerminal open={terminalOpen} onClose={closeTerminal} /></Suspense>
  </>
}
